---
type: Food Product
title: OKmart 靜岡抹茶拿鐵（熱）
description: 從 TWFood 共筆試算表匯入的可追溯營養觀測 draft。
resource: https://foodtracer.health.ntpc.gov.tw/w/foodtracer/FoodPublicInfo?areaId=21120615314152601&infoId=22352
tags:
  - OKmart
  - 現調飲品
  - TWFood 共筆
  - 待人工審核
generated:
  by: twfoodmcp-google-sheet-importer/1.0.0
  at: 2026-08-22T04:54:57.024Z
status: draft
stale_after: 2027-02-13
sources:
  - id: sheet-source-0da17402a6cb
    resource: https://foodtracer.health.ntpc.gov.tw/w/foodtracer/FoodPublicInfo?areaId=21120615314152601&infoId=22352
    title: 靜岡抹茶拿鐵（熱）食品揭露
    source_class: primary_government
    retrieved_at: 2026-08-13T00:00:00+08:00
access:
  classification: public
food:
  id: food:tw:menu:okmart:靜岡抹茶拿鐵熱
  kind: menu_item
  market: TW
  brand: OKmart
  name: 靜岡抹茶拿鐵（熱）
  aliases:
    - 靜岡抹茶拿鐵（熱）
    - OKmart靜岡抹茶拿鐵（熱）
    - OKmart 靜岡抹茶拿鐵（熱）
revision:
  revision_id: google-sheet-2026-08-22-nut-01114
serving:
  description: 大杯480 ml
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
    values:
      energy_kcal: 342
      sugar_g: 40
quality:
  data_quality: government_database
  completeness: partial
  confidence: medium
  calculation_allowed: false
extraction:
  source_system: TWFood Google Sheet
  spreadsheet_id: 16f4_7C9pBo0d9XDhXyRmqIh-MRhSwnGWAT122H9mONY
  sheet_name: Nutrition Data
  record_id: NUT-01114
  evidence_grade: A
  verification_status_as_reported: verified
  value_method: official_food_disclosure
  source_type: government_food_disclosure
  source_publisher: 新北市食材登錄平台
  source_author: 新北市政府衛生局／業者登錄
  source_row:
    record_id: NUT-01114
    food_name: 靜岡抹茶拿鐵（熱）
    brand_or_context: OKmart
    category: 現調飲品
    serving_basis: 大杯480 ml
    calories_kcal: 342
    protein_g: null
    fat_g: null
    saturated_fat_g: null
    trans_fat_g: null
    carbohydrate_g: null
    sugar_g: 40
    sodium_mg: null
    price_twd: null
    value_method: official_food_disclosure
    evidence_grade: A
    verification_status: verified
    source_type: government_food_disclosure
    source_publisher: 新北市食材登錄平台
    source_author: 新北市政府衛生局／業者登錄
    source_title: 靜岡抹茶拿鐵（熱）食品揭露
    source_url: https://foodtracer.health.ntpc.gov.tw/w/foodtracer/FoodPublicInfo?areaId=21120615314152601&infoId=22352
    article_updated_at: null
    accessed_at: 46247
    primary_source_url: https://foodtracer.health.ntpc.gov.tw/w/foodtracer/FoodPublicInfo?areaId=21120615314152601&infoId=22352
    usage_note: 政府食材登錄頁顯示大杯480ml、熱量342大卡與糖40克；頁面其他營養欄位未能明確讀取者保持空白。
source_observation:
  usage_note: 政府食材登錄頁顯示大杯480ml、熱量342大卡與糖40克；頁面其他營養欄位未能明確讀取者保持空白。
official_review_hint:
  status: not_compared_or_no_match
limitations:
  - 此文件保存試算表的一筆觀測，不表示品牌、政府或 TWFoodMCP 已人工確認其正確性。
  - 試算表中的 verification_status 只作為原始欄位保留，不會轉成 OKF verified 或 human-reviewed。
  - 此 draft 的 quality.calculation_allowed 固定為 false；缺少的營養欄位維持 unknown。
  - 份量描述依原始試算表保存，未據此推算重量、容量或每 100 g／ml 數值。
---

# Summary

此文件保存試算表記錄 NUT-01114 的營養觀測。[^sheet-source-0da17402a6cb] 它是未驗證 draft，不能直接用於營養計算。

# Observation

| 欄位 | 值 |
| --- | --- |
| 食品 | 靜岡抹茶拿鐵（熱） |
| 品牌／情境 | OKmart |
| 分類 | 現調飲品 |
| 份量基準 | 大杯480 ml |
| energy_kcal | 342 |
| sugar_g | 40 |

# Review Required

升為 stable 前，真人 reviewer 必須確認產品身分、份量基準、來源版本、營養數值與重複／改版關係。

[^sheet-source-0da17402a6cb]: 靜岡抹茶拿鐵（熱）食品揭露
