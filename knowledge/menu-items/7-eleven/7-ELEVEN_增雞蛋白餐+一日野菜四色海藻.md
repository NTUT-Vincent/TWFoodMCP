---
type: Food Product
title: 7-ELEVEN 增雞蛋白餐＋一日野菜四色海藻
description: 從 TWFood 共筆試算表匯入的可追溯營養觀測 draft。
resource: https://www.yuvog.com.tw/blog_detail/healthy-convenience-store-meals
tags:
  - 7-ELEVEN
  - 文章晚餐組合
  - TWFood 共筆
  - 待人工審核
generated:
  by: twfoodmcp-google-sheet-importer/1.0.0
  at: 2026-08-22T04:54:57.024Z
status: draft
stale_after: 2027-02-05
sources:
  - id: sheet-source-5ec1a1247e42
    resource: https://www.yuvog.com.tw/blog_detail/healthy-convenience-store-meals
    title: 便利商店健康餐怎麼選？7-11、全家三餐減脂菜單推薦！
    source_class: expert_interpretation
    last_modified: 2024-08-16T00:00:00+08:00
    retrieved_at: 2026-08-05T00:00:00+08:00
access:
  classification: public
food:
  id: food:tw:menu:7-eleven:增雞蛋白餐一日野菜四色海藻
  kind: menu_item
  market: TW
  brand: 7-ELEVEN
  name: 增雞蛋白餐＋一日野菜四色海藻
  aliases:
    - 增雞蛋白餐＋一日野菜四色海藻
    - 7-ELEVEN增雞蛋白餐＋一日野菜四色海藻
    - 7-ELEVEN 增雞蛋白餐＋一日野菜四色海藻
revision:
  revision_id: google-sheet-2026-08-22-nut-00347
serving:
  description: 1組（文章表格完整搭配）
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
    values:
      energy_kcal: 637
quality:
  data_quality: third_party_database
  completeness: minimal
  confidence: low
  calculation_allowed: false
extraction:
  source_system: TWFood Google Sheet
  spreadsheet_id: 16f4_7C9pBo0d9XDhXyRmqIh-MRhSwnGWAT122H9mONY
  sheet_name: Nutrition Data
  record_id: NUT-00347
  evidence_grade: D
  verification_status_as_reported: historical_unverified
  value_method: transcribed_from_article
  source_type: nutrition_article
  source_publisher: 妤果生技
  source_author: null
  source_row:
    record_id: NUT-00347
    food_name: 增雞蛋白餐＋一日野菜四色海藻
    brand_or_context: 7-ELEVEN
    category: 文章晚餐組合
    serving_basis: 1組（文章表格完整搭配）
    calories_kcal: 637
    protein_g: null
    fat_g: null
    saturated_fat_g: null
    trans_fat_g: null
    carbohydrate_g: null
    sugar_g: null
    sodium_mg: null
    price_twd: null
    value_method: transcribed_from_article
    evidence_grade: D
    verification_status: historical_unverified
    source_type: nutrition_article
    source_publisher: 妤果生技
    source_author: null
    source_title: 便利商店健康餐怎麼選？7-11、全家三餐減脂菜單推薦！
    source_url: https://www.yuvog.com.tw/blog_detail/healthy-convenience-store-meals
    article_updated_at: 45520
    accessed_at: 46239
    primary_source_url: null
    usage_note: 採文章表格總熱量；文章敘述與表格的少數總值不一致，因此以表格為準並降級。文中稱主餐提供超過 30g 蛋白質，但未提供完整組合精確蛋白質總值。 商品可能已更新或停產。
source_observation:
  usage_note: 採文章表格總熱量；文章敘述與表格的少數總值不一致，因此以表格為準並降級。文中稱主餐提供超過 30g 蛋白質，但未提供完整組合精確蛋白質總值。 商品可能已更新或停產。
official_review_hint:
  status: not_compared_or_no_match
limitations:
  - 此文件保存試算表的一筆觀測，不表示品牌、政府或 TWFoodMCP 已人工確認其正確性。
  - 試算表中的 verification_status 只作為原始欄位保留，不會轉成 OKF verified 或 human-reviewed。
  - 此 draft 的 quality.calculation_allowed 固定為 false；缺少的營養欄位維持 unknown。
  - 份量描述依原始試算表保存，未據此推算重量、容量或每 100 g／ml 數值。
---

# Summary

此文件保存試算表記錄 NUT-00347 的營養觀測。[^sheet-source-5ec1a1247e42] 它是未驗證 draft，不能直接用於營養計算。

# Observation

| 欄位 | 值 |
| --- | --- |
| 食品 | 增雞蛋白餐＋一日野菜四色海藻 |
| 品牌／情境 | 7-ELEVEN |
| 分類 | 文章晚餐組合 |
| 份量基準 | 1組（文章表格完整搭配） |
| energy_kcal | 637 |

# Review Required

升為 stable 前，真人 reviewer 必須確認產品身分、份量基準、來源版本、營養數值與重複／改版關係。

[^sheet-source-5ec1a1247e42]: 便利商店健康餐怎麼選？7-11、全家三餐減脂菜單推薦！
