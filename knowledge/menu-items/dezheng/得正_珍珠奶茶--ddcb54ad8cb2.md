...
food:
  id: food:tw:menu:dezheng:珍珠奶茶-ddcb54ad8cb2
  kind: menu_item
  market: TW
  brand: 得正
  name: 珍珠奶茶
  aliases:
    ...
    - 得正 珍珠奶茶
revision:
  revision_id: google-sheet-2026-08-22-nut-00549
serving:
  description: 大杯（全糖）
  amount: 1
  unit: serving
...
extraction:
  ...
  source_row:
    ...
    usage_note: 文章表格為全糖、去冰（奶蓋為全糖、少冰）的最高參考值；咖啡因以顏色分級「紅標」呈現，未換算為毫克。
source_observation:
  usage_note: 文章表格為全糖、去冰（奶蓋為全糖、少冰）的最高參考值；咖啡因以顏色分級「紅標」呈現，未換算為毫克。
...
此文件保存試算表記錄 NUT-00549 的營養觀測。[^sheet-source-4c2f05ad720f] 它是未驗證 draft，不能直接用於營養計算。
