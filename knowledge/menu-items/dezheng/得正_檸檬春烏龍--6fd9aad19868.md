---
type: Food Product
title: 得正 檸檬春烏龍
description: 從 TWFood 共筆試算表匯入的可追溯營養觀測 draft。
resource: https://dailydietitian.com.tw/%E5%BE%97%E6%AD%A3%E8%8F%9C%E5%96%AE-%E7%86%B1%E9%87%8F-%E7%B3%96%E9%87%8F-%E5%92%96%E5%95%A1%E5%9B%A0-%E5%83%B9%E7%9B%AE%E8%A1%A8/
tags:
  - 得正
  - 水果茶
  - TWFood 共筆
  - 待人工審核
generated:
  by: twfoodmcp-google-sheet-importer/1.0.0
  at: 2026-08-22T04:54:57.024Z
status: draft
stale_after: 2027-02-05
sources:
  - id: sheet-source-4c2f05ad720f
    resource: https://dailydietitian.com.tw/%E5%BE%97%E6%AD%A3%E8%8F%9C%E5%96%AE-%E7%86%B1%E9%87%8F-%E7%B3%96%E9%87%8F-%E5%92%96%E5%95%A1%E5%9B%A0-%E5%83%B9%E7%9B%AE%E8%A1%A8/
    title: 2026 得正菜單｜熱量、糖量、咖啡因、價目表＋營養師手搖點餐建議
    source_class: expert_interpretation
    last_modified: 2026-07-05T00:00:00+08:00
    retrieved_at: 2026-08-05T00:00:00+08:00
access:
  classification: public
food:
  id: food:tw:menu:dezheng:檸檬春烏龍-6fd9aad19868
  kind: menu_item
  market: TW
  brand: 得正
  name: 檸檬春烏龍
  aliases:
    - 檸檬春烏龍
    - 得正檸檬春烏龍
    - 得正 檸檬春烏龍
revision:
  revision_id: google-sheet-2026-08-22-nut-00533
serving:
  description: 大杯（全糖）
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
    values:
      energy_kcal: 267
      sugar_g: 58
quality:
  data_quality: third_party_database
  completeness: partial
  confidence: low
  calculation_allowed: false
extraction:
  source_system: TWFood Google Sheet
  spreadsheet_id: 16f4_7C9pBo0d9XDhXyRmqIh-MRhSwnGWAT122H9mONY
  sheet_name: Nutrition Data
  record_id: NUT-00533
  evidence_grade: C
  verification_status_as_reported: unverified
  value_method: transcribed_from_article
  source_type: dietitian_article
  source_publisher: 日日營養 DailyDietitian
  source_author: 日日營養團隊
  source_row:
    record_id: NUT-00533
    food_name: 檸檬春烏龍
    brand_or_context: 得正
    category: 水果茶
    serving_basis: 大杯（全糖）
    calories_kcal: 267
    protein_g: null
    fat_g: null
    saturated_fat_g: null
    trans_fat_g: null
    carbohydrate_g: null
    sugar_g: 58
    sodium_mg: null
    price_twd: null
    value_method: transcribed_from_article
    evidence_grade: C
    verification_status: unverified
    source_type: dietitian_article
    source_publisher: 日日營養 DailyDietitian
    source_author: 日日營養團隊
    source_title: 2026 得正菜單｜熱量、糖量、咖啡因、價目表＋營養師手搖點餐建議
    source_url: https://dailydietitian.com.tw/%E5%BE%97%E6%AD%A3%E8%8F%9C%E5%96%AE-%E7%86%B1%E9%87%8F-%E7%B3%96%E9%87%8F-%E5%92%96%E5%95%A1%E5%9B%A0-%E5%83%B9%E7%9B%AE%E8%A1%A8/
    article_updated_at: 46208
    accessed_at: 46239
    primary_source_url: null
    usage_note: 文章表格為全糖、去冰（奶蓋為全糖、少冰）的最高參考值；咖啡因以顏色分級「綠/黃」呈現，未換算為毫克。
source_observation:
  usage_note: 文章表格為全糖、去冰（奶蓋為全糖、少冰）的最高參考值；咖啡因以顏色分級「綠/黃」呈現，未換算為毫克。
official_review_hint:
  status: not_compared_or_no_match
limitations:
  - 此文件保存試算表的一筆觀測，不表示品牌、政府或 TWFoodMCP 已人工確認其正確性。
  - 試算表中的 verification_status 只作為原始欄位保留，不會轉成 OKF verified 或 human-reviewed。
  - 此 draft 的 quality.calculation_allowed 固定為 false；缺少的營養欄位維持 unknown。
  - 份量描述依原始試算表保存，未據此推算重量、容量或每 100 g／ml 數值。
---

# Summary

此文件保存試算表記錄 NUT-00533 的營養觀測。[^sheet-source-4c2f05ad720f] 它是未驗證 draft，不能直接用於營養計算。

# Observation

| 欄位 | 值 |
| --- | --- |
| 食品 | 檸檬春烏龍 |
| 品牌／情境 | 得正 |
| 分類 | 水果茶 |
| 份量基準 | 大杯（全糖） |
| energy_kcal | 267 |
| sugar_g | 58 |

# Review Required

升為 stable 前，真人 reviewer 必須確認產品身分、份量基準、來源版本、營養數值與重複／改版關係。

[^sheet-source-4c2f05ad720f]: 2026 得正菜單｜熱量、糖量、咖啡因、價目表＋營養師手搖點餐建議
