...
food:
  id: food:tw:menu:dezheng:芝士奶蓋緋烏龍-c6530c52d01a
  kind: menu_item
  market: TW
  brand: 得正
  name: 芝士奶蓋緋烏龍
  aliases:
    ...
    - 得正 芝士奶蓋緋烏龍
revision:
  revision_id: google-sheet-2026-08-22-nut-00575
serving:
  description: 大杯（全糖）
  amount: 1
  unit: serving
...
extraction:
  ...
  source_row:
    ...
    usage_note: 文章表格為全糖、去冰（奶蓋為全糖、少冰）的最高參考值；咖啡因以顏色分級「綠/黃」呈現，未換算為毫克。
source_observation:
  usage_note: 文章表格為全糖、去冰（奶蓋為全糖、少冰）的最高參考值；咖啡因以顏色分級「綠/黃」呈現，未換算為毫克。
...
此文件保存試算表記錄 NUT-00575 的營養觀測。[^sheet-source-4c2f05ad720f] 它是未驗證 draft，不能直接用於營養計算。
