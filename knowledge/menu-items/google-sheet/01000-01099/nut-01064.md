---
type: Food Product
title: 萊爾富 Hi-Life 健身雞肉蛋白餐
description: 從 TWFood 共筆試算表匯入的可追溯營養觀測 draft。
resource: https://www.foodnext.net/life/recipes/dessert/paper/5470908183
tags:
  - 萊爾富 Hi-Life
  - 蛋白餐盒
  - TWFood 共筆
  - 待人工審核
generated:
  by: twfoodmcp-google-sheet-importer/1.0.0
  at: 2026-08-22T04:54:57.024Z
status: draft
stale_after: 2027-02-10
sources:
  - id: sheet-source-91e20b349fca
    resource: https://www.foodnext.net/life/recipes/dessert/paper/5470908183
    title: 萊爾富歡慶35週年推出4款營養師聯名健康輕食
    source_class: third_party_dataset
    last_modified: 2024-02-21T00:00:00+08:00
    retrieved_at: 2026-08-10T00:00:00+08:00
access:
  classification: public
food:
  id: food:tw:menu:google-sheet:nut-01064
  kind: menu_item
  market: TW
  brand: 萊爾富 Hi-Life
  name: 健身雞肉蛋白餐
  aliases:
    - 健身雞肉蛋白餐
    - 萊爾富 Hi-Life健身雞肉蛋白餐
    - 萊爾富 Hi-Life 健身雞肉蛋白餐
revision:
  revision_id: google-sheet-2026-08-22-nut-01064
serving:
  description: 1盒（文章列示）
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
    values:
      protein_g: 35.1
quality:
  data_quality: third_party_database
  completeness: minimal
  confidence: low
  calculation_allowed: false
extraction:
  source_system: TWFood Google Sheet
  spreadsheet_id: 16f4_7C9pBo0d9XDhXyRmqIh-MRhSwnGWAT122H9mONY
  sheet_name: Nutrition Data
  record_id: NUT-01064
  evidence_grade: D
  verification_status_as_reported: historical_unverified
  value_method: transcribed_from_article
  source_type: news_article
  source_publisher: 食力 foodNEXT
  source_author: null
  source_row:
    record_id: NUT-01064
    food_name: 健身雞肉蛋白餐
    brand_or_context: 萊爾富 Hi-Life
    category: 蛋白餐盒
    serving_basis: 1盒（文章列示）
    calories_kcal: null
    protein_g: 35.1
    fat_g: null
    saturated_fat_g: null
    trans_fat_g: null
    carbohydrate_g: null
    sugar_g: null
    sodium_mg: null
    price_twd: 95
    value_method: transcribed_from_article
    evidence_grade: D
    verification_status: historical_unverified
    source_type: news_article
    source_publisher: 食力 foodNEXT
    source_author: null
    source_title: 萊爾富歡慶35週年推出4款營養師聯名健康輕食
    source_url: https://www.foodnext.net/life/recipes/dessert/paper/5470908183
    article_updated_at: 45343
    accessed_at: 46244
    primary_source_url: null
    usage_note: 2024年媒體文章轉述萊爾富與營養師合作商品，只收明確蛋白質與價格；未提供完整營養標示，列歷史待驗證。
source_observation:
  price_twd: 95
  usage_note: 2024年媒體文章轉述萊爾富與營養師合作商品，只收明確蛋白質與價格；未提供完整營養標示，列歷史待驗證。
official_review_hint:
  status: not_compared_or_no_match
limitations:
  - 此文件保存試算表的一筆觀測，不表示品牌、政府或 TWFoodMCP 已人工確認其正確性。
  - 試算表中的 verification_status 只作為原始欄位保留，不會轉成 OKF verified 或 human-reviewed。
  - 此 draft 的 quality.calculation_allowed 固定為 false；缺少的營養欄位維持 unknown。
  - 份量描述依原始試算表保存，未據此推算重量、容量或每 100 g／ml 數值。
---

# Summary

此文件保存試算表記錄 NUT-01064 的營養觀測。[^sheet-source-91e20b349fca] 它是未驗證 draft，不能直接用於營養計算。

# Observation

| 欄位 | 值 |
| --- | --- |
| 食品 | 健身雞肉蛋白餐 |
| 品牌／情境 | 萊爾富 Hi-Life |
| 分類 | 蛋白餐盒 |
| 份量基準 | 1盒（文章列示） |
| protein_g | 35.1 |

# Review Required

升為 stable 前，真人 reviewer 必須確認產品身分、份量基準、來源版本、營養數值與重複／改版關係。

[^sheet-source-91e20b349fca]: 萊爾富歡慶35週年推出4款營養師聯名健康輕食
