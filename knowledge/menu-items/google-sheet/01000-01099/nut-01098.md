---
type: Food Product
title: 萊爾富 Hi-Life 韓味雞胸蛋白餐
description: 從 TWFood 共筆試算表匯入的可追溯營養觀測 draft。
resource: https://vocus.cc/post/69a98cf9fd89780001429f18
tags:
  - 萊爾富 Hi-Life
  - 蛋白餐盒
  - TWFood 共筆
  - 待人工審核
generated:
  by: twfoodmcp-google-sheet-importer/1.0.0
  at: 2026-08-22T04:54:57.024Z
status: draft
stale_after: 2027-02-12
sources:
  - id: sheet-source-9b36b0b2af15
    resource: https://vocus.cc/post/69a98cf9fd89780001429f18
    title: 萊爾富韓味食堂×肌肉山山｜這波真的有認真
    source_class: third_party_dataset
    last_modified: 2026-03-05T00:00:00+08:00
    retrieved_at: 2026-08-12T00:00:00+08:00
access:
  classification: public
food:
  id: food:tw:menu:google-sheet:nut-01098
  kind: menu_item
  market: TW
  brand: 萊爾富 Hi-Life
  name: 韓味雞胸蛋白餐
  aliases:
    - 韓味雞胸蛋白餐
    - 萊爾富 Hi-Life韓味雞胸蛋白餐
    - 萊爾富 Hi-Life 韓味雞胸蛋白餐
revision:
  revision_id: google-sheet-2026-08-22-nut-01098
serving:
  description: 1盒
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
    values:
      energy_kcal: 577.8
      protein_g: 29.9
quality:
  data_quality: third_party_database
  completeness: partial
  confidence: low
  calculation_allowed: false
extraction:
  source_system: TWFood Google Sheet
  spreadsheet_id: 16f4_7C9pBo0d9XDhXyRmqIh-MRhSwnGWAT122H9mONY
  sheet_name: Nutrition Data
  record_id: NUT-01098
  evidence_grade: D
  verification_status_as_reported: unverified
  value_method: transcribed_from_article
  source_type: blog_article
  source_publisher: 方格子 vocus
  source_author: 水電工阿嫻
  source_row:
    record_id: NUT-01098
    food_name: 韓味雞胸蛋白餐
    brand_or_context: 萊爾富 Hi-Life
    category: 蛋白餐盒
    serving_basis: 1盒
    calories_kcal: 577.8
    protein_g: 29.9
    fat_g: null
    saturated_fat_g: null
    trans_fat_g: null
    carbohydrate_g: null
    sugar_g: null
    sodium_mg: null
    price_twd: 99
    value_method: transcribed_from_article
    evidence_grade: D
    verification_status: unverified
    source_type: blog_article
    source_publisher: 方格子 vocus
    source_author: 水電工阿嫻
    source_title: 萊爾富韓味食堂×肌肉山山｜這波真的有認真
    source_url: https://vocus.cc/post/69a98cf9fd89780001429f18
    article_updated_at: 46086
    accessed_at: 46246
    primary_source_url: null
    usage_note: 2026年一般部落格文章轉錄萊爾富韓味食堂商品熱量／蛋白質與價格；未取得包裝完整標示，故列D級待驗證，不視為官方值。
source_observation:
  price_twd: 99
  usage_note: 2026年一般部落格文章轉錄萊爾富韓味食堂商品熱量／蛋白質與價格；未取得包裝完整標示，故列D級待驗證，不視為官方值。
official_review_hint:
  status: not_compared_or_no_match
limitations:
  - 此文件保存試算表的一筆觀測，不表示品牌、政府或 TWFoodMCP 已人工確認其正確性。
  - 試算表中的 verification_status 只作為原始欄位保留，不會轉成 OKF verified 或 human-reviewed。
  - 此 draft 的 quality.calculation_allowed 固定為 false；缺少的營養欄位維持 unknown。
  - 份量描述依原始試算表保存，未據此推算重量、容量或每 100 g／ml 數值。
---

# Summary

此文件保存試算表記錄 NUT-01098 的營養觀測。[^sheet-source-9b36b0b2af15] 它是未驗證 draft，不能直接用於營養計算。

# Observation

| 欄位 | 值 |
| --- | --- |
| 食品 | 韓味雞胸蛋白餐 |
| 品牌／情境 | 萊爾富 Hi-Life |
| 分類 | 蛋白餐盒 |
| 份量基準 | 1盒 |
| energy_kcal | 577.8 |
| protein_g | 29.9 |

# Review Required

升為 stable 前，真人 reviewer 必須確認產品身分、份量基準、來源版本、營養數值與重複／改版關係。

[^sheet-source-9b36b0b2af15]: 萊爾富韓味食堂×肌肉山山｜這波真的有認真
