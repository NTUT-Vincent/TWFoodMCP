---
type: Food Product
title: 萊爾富 Hi-Life 生乳你個布丁
description: 從 TWFood 共筆試算表匯入的可追溯營養觀測 draft。
resource: https://www.threads.com/@nervous_rd/post/DYMqS88E-8_
tags:
  - 萊爾富 Hi-Life
  - 甜點／布丁
  - TWFood 共筆
  - 待人工審核
generated:
  by: twfoodmcp-google-sheet-importer/1.0.0
  at: 2026-08-22T04:54:57.024Z
status: draft
stale_after: 2027-02-19
sources:
  - id: sheet-source-fd8f31ba6ec6
    resource: https://www.threads.com/@nervous_rd/post/DYMqS88E-8_
    title: 萊爾富生乳你個布丁營養分享
    source_class: community_submission
    last_modified: 2026-05-11T00:00:00+08:00
    retrieved_at: 2026-08-19T00:00:00+08:00
access:
  classification: public
food:
  id: food:tw:menu:google-sheet:nut-01191
  kind: menu_item
  market: TW
  brand: 萊爾富 Hi-Life
  name: 生乳你個布丁
  aliases:
    - 生乳你個布丁
    - 萊爾富 Hi-Life生乳你個布丁
    - 萊爾富 Hi-Life 生乳你個布丁
revision:
  revision_id: google-sheet-2026-08-22-nut-01191
serving:
  description: 1個（完整商品）
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
    values:
      energy_kcal: 241.5
      protein_g: 6.9
      fat_g: 15.9
      carbohydrate_g: 17.7
quality:
  data_quality: community_report
  completeness: partial
  confidence: low
  calculation_allowed: false
extraction:
  source_system: TWFood Google Sheet
  spreadsheet_id: 16f4_7C9pBo0d9XDhXyRmqIh-MRhSwnGWAT122H9mONY
  sheet_name: Nutrition Data
  record_id: NUT-01191
  evidence_grade: D
  verification_status_as_reported: unverified
  value_method: transcribed_from_social_post
  source_type: social_post
  source_publisher: Threads
  source_author: 緊張營養師 nervous_rd
  source_row:
    record_id: NUT-01191
    food_name: 生乳你個布丁
    brand_or_context: 萊爾富 Hi-Life
    category: 甜點／布丁
    serving_basis: 1個（完整商品）
    calories_kcal: 241.5
    protein_g: 6.9
    fat_g: 15.9
    saturated_fat_g: null
    trans_fat_g: null
    carbohydrate_g: 17.7
    sugar_g: null
    sodium_mg: null
    price_twd: null
    value_method: transcribed_from_social_post
    evidence_grade: D
    verification_status: unverified
    source_type: social_post
    source_publisher: Threads
    source_author: 緊張營養師 nervous_rd
    source_title: 萊爾富生乳你個布丁營養分享
    source_url: https://www.threads.com/@nervous_rd/post/DYMqS88E-8_
    article_updated_at: 46153
    accessed_at: 46253
    primary_source_url: null
    usage_note: 營養師社群貼文明示熱量與三大營養素；糖與鈉未提供，保持空白。
source_observation:
  usage_note: 營養師社群貼文明示熱量與三大營養素；糖與鈉未提供，保持空白。
official_review_hint:
  status: not_compared_or_no_match
limitations:
  - 此文件保存試算表的一筆觀測，不表示品牌、政府或 TWFoodMCP 已人工確認其正確性。
  - 試算表中的 verification_status 只作為原始欄位保留，不會轉成 OKF verified 或 human-reviewed。
  - 此 draft 的 quality.calculation_allowed 固定為 false；缺少的營養欄位維持 unknown。
  - 份量描述依原始試算表保存，未據此推算重量、容量或每 100 g／ml 數值。
---

# Summary

此文件保存試算表記錄 NUT-01191 的營養觀測。[^sheet-source-fd8f31ba6ec6] 它是未驗證 draft，不能直接用於營養計算。

# Observation

| 欄位 | 值 |
| --- | --- |
| 食品 | 生乳你個布丁 |
| 品牌／情境 | 萊爾富 Hi-Life |
| 分類 | 甜點／布丁 |
| 份量基準 | 1個（完整商品） |
| energy_kcal | 241.5 |
| protein_g | 6.9 |
| fat_g | 15.9 |
| carbohydrate_g | 17.7 |

# Review Required

升為 stable 前，真人 reviewer 必須確認產品身分、份量基準、來源版本、營養數值與重複／改版關係。

[^sheet-source-fd8f31ba6ec6]: 萊爾富生乳你個布丁營養分享
