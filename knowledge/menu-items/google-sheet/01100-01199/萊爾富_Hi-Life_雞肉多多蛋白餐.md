---
type: Food Product
title: 萊爾富 Hi-Life 雞肉多多蛋白餐
description: 從 TWFood 共筆試算表匯入的可追溯營養觀測 draft。
resource: https://www.threads.com/@hi_frieda.c/post/DMXj95IhXLU
tags:
  - 萊爾富 Hi-Life
  - 餐盒
  - TWFood 共筆
  - 待人工審核
generated:
  by: twfoodmcp-google-sheet-importer/1.0.0
  at: 2026-08-22T04:54:57.024Z
status: draft
stale_after: 2027-02-18
sources:
  - id: sheet-source-a115e517a48f
    resource: https://www.threads.com/@hi_frieda.c/post/DMXj95IhXLU
    title: 萊爾富雞肉多多蛋白餐開箱
    source_class: community_submission
    last_modified: 2025-07-21T00:00:00+08:00
    retrieved_at: 2026-08-18T00:00:00+08:00
access:
  classification: public
food:
  id: food:tw:menu:google-sheet:nut-01164
  kind: menu_item
  market: TW
  brand: 萊爾富 Hi-Life
  name: 雞肉多多蛋白餐
  aliases:
    - 雞肉多多蛋白餐
    - 萊爾富 Hi-Life雞肉多多蛋白餐
    - 萊爾富 Hi-Life 雞肉多多蛋白餐
revision:
  revision_id: google-sheet-2026-08-22-nut-01164
serving:
  description: 1盒（完整商品）
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
    values:
      protein_g: 33
quality:
  data_quality: community_report
  completeness: minimal
  confidence: low
  calculation_allowed: false
extraction:
  source_system: TWFood Google Sheet
  spreadsheet_id: 16f4_7C9pBo0d9XDhXyRmqIh-MRhSwnGWAT122H9mONY
  sheet_name: Nutrition Data
  record_id: NUT-01164
  evidence_grade: D
  verification_status_as_reported: historical_unverified
  value_method: transcribed_from_social_image
  source_type: social_post
  source_publisher: Threads
  source_author: hi_frieda.c
  source_row:
    record_id: NUT-01164
    food_name: 雞肉多多蛋白餐
    brand_or_context: 萊爾富 Hi-Life
    category: 餐盒
    serving_basis: 1盒（完整商品）
    calories_kcal: null
    protein_g: 33
    fat_g: null
    saturated_fat_g: null
    trans_fat_g: null
    carbohydrate_g: null
    sugar_g: null
    sodium_mg: null
    price_twd: null
    value_method: transcribed_from_social_image
    evidence_grade: D
    verification_status: historical_unverified
    source_type: social_post
    source_publisher: Threads
    source_author: hi_frieda.c
    source_title: 萊爾富雞肉多多蛋白餐開箱
    source_url: https://www.threads.com/@hi_frieda.c/post/DMXj95IhXLU
    article_updated_at: 45859
    accessed_at: 46252
    primary_source_url: null
    usage_note: 社群貼文與搜尋索引可辨識品名及蛋白質33g；未取得其餘清晰標示，現行配方待核對。
source_observation:
  usage_note: 社群貼文與搜尋索引可辨識品名及蛋白質33g；未取得其餘清晰標示，現行配方待核對。
official_review_hint:
  status: not_compared_or_no_match
limitations:
  - 此文件保存試算表的一筆觀測，不表示品牌、政府或 TWFoodMCP 已人工確認其正確性。
  - 試算表中的 verification_status 只作為原始欄位保留，不會轉成 OKF verified 或 human-reviewed。
  - 此 draft 的 quality.calculation_allowed 固定為 false；缺少的營養欄位維持 unknown。
  - 份量描述依原始試算表保存，未據此推算重量、容量或每 100 g／ml 數值。
---

# Summary

此文件保存試算表記錄 NUT-01164 的營養觀測。[^sheet-source-a115e517a48f] 它是未驗證 draft，不能直接用於營養計算。

# Observation

| 欄位 | 值 |
| --- | --- |
| 食品 | 雞肉多多蛋白餐 |
| 品牌／情境 | 萊爾富 Hi-Life |
| 分類 | 餐盒 |
| 份量基準 | 1盒（完整商品） |
| protein_g | 33 |

# Review Required

升為 stable 前，真人 reviewer 必須確認產品身分、份量基準、來源版本、營養數值與重複／改版關係。

[^sheet-source-a115e517a48f]: 萊爾富雞肉多多蛋白餐開箱
