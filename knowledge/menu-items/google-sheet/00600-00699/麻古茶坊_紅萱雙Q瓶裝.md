...
food:
  id: food:tw:menu:google-sheet:nut-00684
  kind: menu_item
  market: TW
  brand: 麻古茶坊
  name: 紅萱雙Q瓶裝
  aliases:
    ...
    - 麻古茶坊 紅萱雙Q瓶裝
revision:
  revision_id: google-sheet-2026-08-22-nut-00684
serving:
  description: 1瓶（文章未列容量／甜度）
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
...
extraction:
  ...
  source_row:
    ...
    usage_note: 文章表格轉錄總熱量與總糖；咖啡因標示 101–200 mg。容量、甜度與冰量未明示，不宜和其他杯型直接比較。
source_observation:
  usage_note: 文章表格轉錄總熱量與總糖；咖啡因標示 101–200 mg。容量、甜度與冰量未明示，不宜和其他杯型直接比較。
...
此文件保存試算表記錄 NUT-00684 的營養觀測。[^sheet-source-466715605c89] 它是未驗證 draft，不能直接用於營養計算。
