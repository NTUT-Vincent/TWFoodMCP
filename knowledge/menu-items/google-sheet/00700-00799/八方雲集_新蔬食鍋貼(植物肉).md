...
food:
  id: food:tw:menu:google-sheet:nut-00723
  kind: menu_item
  market: TW
  brand: 八方雲集
  name: 新蔬食鍋貼（植物肉）
  aliases:
    ...
    - 八方雲集 新蔬食鍋貼（植物肉）
revision:
  revision_id: google-sheet-2026-08-22-nut-00723
serving:
  description: 1顆（文章以約30g估算）
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
...
extraction:
  ...
  source_row:
    ...
    usage_note: 文章由營養師估算單顆熱量；未直接核對現行門市配方或官方營養標示。
source_observation:
  usage_note: 文章由營養師估算單顆熱量；未直接核對現行門市配方或官方營養標示。
...
此文件保存試算表記錄 NUT-00723 的營養觀測。[^sheet-source-d916121be823] 它是未驗證 draft，不能直接用於營養計算。
