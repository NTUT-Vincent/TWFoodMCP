---
type: Food Product
title: OKmart 幸福餐盒雞胸肉口味
description: 從 TWFood 共筆試算表匯入的可追溯營養觀測 draft。
resource: https://www.leepsyclinic.com/2025/07/blog-post_27.html
tags:
  - OKmart
  - 餐盒
  - TWFood 共筆
  - 待人工審核
generated:
  by: twfoodmcp-google-sheet-importer/1.0.0
  at: 2026-08-22T04:54:57.024Z
status: draft
stale_after: 2027-02-07
sources:
  - id: sheet-source-7c7c18b19ad1
    resource: https://www.leepsyclinic.com/2025/07/blog-post_27.html
    title: 猛健樂與週纖達高蛋白質飲食｜線上飲食搭配｜7-11 全家萊爾富 OK超商｜星巴克露易莎
    source_class: expert_interpretation
    last_modified: 2025-07-27T00:00:00+08:00
    retrieved_at: 2026-08-07T00:00:00+08:00
access:
  classification: public
food:
  id: food:tw:menu:google-sheet:nut-00894
  kind: menu_item
  market: TW
  brand: OKmart
  name: 幸福餐盒雞胸肉口味
  aliases:
    - 幸福餐盒雞胸肉口味
    - OKmart幸福餐盒雞胸肉口味
    - OKmart 幸福餐盒雞胸肉口味
revision:
  revision_id: google-sheet-2026-08-22-nut-00894
serving:
  description: 1盒（文章列示）
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
    values:
      energy_kcal: 614
      protein_g: 39.5
quality:
  data_quality: third_party_database
  completeness: partial
  confidence: low
  calculation_allowed: false
extraction:
  source_system: TWFood Google Sheet
  spreadsheet_id: 16f4_7C9pBo0d9XDhXyRmqIh-MRhSwnGWAT122H9mONY
  sheet_name: Nutrition Data
  record_id: NUT-00894
  evidence_grade: C
  verification_status_as_reported: unverified
  value_method: transcribed_from_article
  source_type: medical_article
  source_publisher: 李政洋身心診所
  source_author: null
  source_row:
    record_id: NUT-00894
    food_name: 幸福餐盒雞胸肉口味
    brand_or_context: OKmart
    category: 餐盒
    serving_basis: 1盒（文章列示）
    calories_kcal: 614
    protein_g: 39.5
    fat_g: null
    saturated_fat_g: null
    trans_fat_g: null
    carbohydrate_g: null
    sugar_g: null
    sodium_mg: null
    price_twd: null
    value_method: transcribed_from_article
    evidence_grade: C
    verification_status: unverified
    source_type: medical_article
    source_publisher: 李政洋身心診所
    source_author: null
    source_title: 猛健樂與週纖達高蛋白質飲食｜線上飲食搭配｜7-11 全家萊爾富 OK超商｜星巴克露易莎
    source_url: https://www.leepsyclinic.com/2025/07/blog-post_27.html
    article_updated_at: 2025-07-27
    accessed_at: 2026-08-07
    primary_source_url: null
    usage_note: 診所文章在 OK超商段落列示熱量與蛋白質；未提供可追溯原始營養標示，列未驗證。
source_observation:
  usage_note: 診所文章在 OK超商段落列示熱量與蛋白質；未提供可追溯原始營養標示，列未驗證。
official_review_hint:
  status: not_compared_or_no_match
limitations:
  - 此文件保存試算表的一筆觀測，不表示品牌、政府或 TWFoodMCP 已人工確認其正確性。
  - 試算表中的 verification_status 只作為原始欄位保留，不會轉成 OKF verified 或 human-reviewed。
  - 此 draft 的 quality.calculation_allowed 固定為 false；缺少的營養欄位維持 unknown。
  - 份量描述依原始試算表保存，未據此推算重量、容量或每 100 g／ml 數值。
---

# Summary

此文件保存試算表記錄 NUT-00894 的營養觀測。[^sheet-source-7c7c18b19ad1] 它是未驗證 draft，不能直接用於營養計算。

# Observation

| 欄位 | 值 |
| --- | --- |
| 食品 | 幸福餐盒雞胸肉口味 |
| 品牌／情境 | OKmart |
| 分類 | 餐盒 |
| 份量基準 | 1盒（文章列示） |
| energy_kcal | 614 |
| protein_g | 39.5 |

# Review Required

升為 stable 前，真人 reviewer 必須確認產品身分、份量基準、來源版本、營養數值與重複／改版關係。

[^sheet-source-7c7c18b19ad1]: 猛健樂與週纖達高蛋白質飲食｜線上飲食搭配｜7-11 全家萊爾富 OK超商｜星巴克露易莎
