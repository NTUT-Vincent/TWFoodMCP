---
type: Food Product
title: 台灣麥當勞 McDonald's 義式濃縮咖啡(熱)
...
food:
  id: food:tw:menu:mcdonalds:mcdonalds_義式濃縮咖啡熱
  kind: menu_item
  market: TW
  ...
  aliases:
    - 義式濃縮咖啡(熱)
    - 台灣麥當勞 McDonald's義式濃縮咖啡(熱)
    - 台灣麥當勞 McDonald's 義式濃縮咖啡(熱)
revision:
  revision_id: google-sheet-2026-08-22-nut-00040
serving:
...
official_review_hint:
  ...
  existing_food_id: food:tw:menu:mcdonalds:espresso
  existing_title: 麥當勞 義式濃縮咖啡(熱)
  existing_concept: /menu-items/mcdonalds/espresso.md
  comparison:
    corroborated: true
    compared:
      - &a1
        field: energy_kcal
        candidate_value: 6.26
        official_value: 6.26
        delta: 0
        tolerance: 1
      - &a2
...
此文件保存試算表記錄 NUT-00040 的營養觀測。[^sheet-source-170fcc78d889] 它是未驗證 draft，不能直接用於營養計算。
...
數值可交叉支持：[麥當勞 義式濃縮咖啡(熱)](/menu-items/mcdonalds/espresso.md)。
