---
type: Food Product
title: 台灣麥當勞 McDonald's 經典美式咖啡(冰)
...
food:
  id: food:tw:menu:mcdonalds:mcdonalds_經典美式咖啡冰
  kind: menu_item
  market: TW
  ...
  aliases:
    - 經典美式咖啡(冰)
    - 台灣麥當勞 McDonald's經典美式咖啡(冰)
    - 台灣麥當勞 McDonald's 經典美式咖啡(冰)
revision:
  revision_id: google-sheet-2026-08-22-nut-00038
serving:
...
official_review_hint:
  ...
  existing_food_id: food:tw:menu:mcdonalds:iced-americano
  existing_title: 麥當勞 經典美式咖啡(冰)
  existing_concept: /menu-items/mcdonalds/iced-americano.md
  comparison:
    corroborated: true
    compared:
      - &a1
        field: energy_kcal
        candidate_value: 20.16
        official_value: 20.16
        delta: 0
        tolerance: 1
      - &a2
        field: protein_g
...
此文件保存試算表記錄 NUT-00038 的營養觀測。[^sheet-source-170fcc78d889] 它是未驗證 draft，不能直接用於營養計算。
...
數值可交叉支持：[麥當勞 經典美式咖啡(冰)](/menu-items/mcdonalds/iced-americano.md)。
