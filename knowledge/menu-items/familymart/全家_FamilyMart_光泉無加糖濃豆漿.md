---
type: Food Product
title: 全家 FamilyMart 光泉無加糖濃豆漿
description: 從 TWFood 共筆試算表匯入的可追溯營養觀測 draft。
resource: https://foodsafety.family.com.tw/Web_FFD_2022/product/2946091
tags:
  - 全家 FamilyMart
  - 豆漿
  - TWFood 共筆
  - 待人工審核
generated:
  by: twfoodmcp-google-sheet-importer/1.0.0
  at: 2026-08-22T04:54:57.024Z
status: draft
stale_after: 2027-02-08
sources:
  - id: sheet-source-7ae5a08dc447
    resource: https://foodsafety.family.com.tw/Web_FFD_2022/product/2946091
    title: 光泉無加糖濃豆漿－全家食在購安心
    source_class: primary_official
    retrieved_at: 2026-08-08T00:00:00+08:00
access:
  classification: public
food:
  id: food:tw:menu:familymart:familymart_光泉無加糖濃豆漿
  kind: menu_item
  market: TW
  brand: 全家 FamilyMart
  name: 光泉無加糖濃豆漿
  aliases:
    - 光泉無加糖濃豆漿
    - 全家 FamilyMart光泉無加糖濃豆漿
    - 全家 FamilyMart 光泉無加糖濃豆漿
revision:
  revision_id: google-sheet-2026-08-22-nut-00994
serving:
  description: 每份375毫升（本包裝含1份）
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
    values:
      energy_kcal: 190.2
      protein_g: 19.1
      fat_g: 9.8
      carbohydrate_g: 7.9
quality:
  data_quality: official_brand
  completeness: partial
  confidence: medium
  calculation_allowed: false
extraction:
  source_system: TWFood Google Sheet
  spreadsheet_id: 16f4_7C9pBo0d9XDhXyRmqIh-MRhSwnGWAT122H9mONY
  sheet_name: Nutrition Data
  record_id: NUT-00994
  evidence_grade: A
  verification_status_as_reported: official_page
  value_method: official_product_label
  source_type: brand_official_nutrition_page
  source_publisher: 全家便利商店
  source_author: null
  source_row:
    record_id: NUT-00994
    food_name: 光泉無加糖濃豆漿
    brand_or_context: 全家 FamilyMart
    category: 豆漿
    serving_basis: 每份375毫升（本包裝含1份）
    calories_kcal: 190.2
    protein_g: 19.1
    fat_g: 9.8
    saturated_fat_g: null
    trans_fat_g: null
    carbohydrate_g: 7.9
    sugar_g: null
    sodium_mg: null
    price_twd: null
    value_method: official_product_label
    evidence_grade: A
    verification_status: official_page
    source_type: brand_official_nutrition_page
    source_publisher: 全家便利商店
    source_author: null
    source_title: 光泉無加糖濃豆漿－全家食在購安心
    source_url: https://foodsafety.family.com.tw/Web_FFD_2022/product/2946091
    article_updated_at: null
    accessed_at: 46242
    primary_source_url: null
    usage_note: 全家官方食安頁的每份規格與熱量、蛋白質、脂肪、碳水化合物；未呈現的營養素保持空白，實際包裝優先。
source_observation:
  usage_note: 全家官方食安頁的每份規格與熱量、蛋白質、脂肪、碳水化合物；未呈現的營養素保持空白，實際包裝優先。
official_review_hint:
  status: not_compared_or_no_match
limitations:
  - 此文件保存試算表的一筆觀測，不表示品牌、政府或 TWFoodMCP 已人工確認其正確性。
  - 試算表中的 verification_status 只作為原始欄位保留，不會轉成 OKF verified 或 human-reviewed。
  - 此 draft 的 quality.calculation_allowed 固定為 false；缺少的營養欄位維持 unknown。
  - 份量描述依原始試算表保存，未據此推算重量、容量或每 100 g／ml 數值。
---

# Summary

此文件保存試算表記錄 NUT-00994 的營養觀測。[^sheet-source-7ae5a08dc447] 它是未驗證 draft，不能直接用於營養計算。

# Observation

| 欄位 | 值 |
| --- | --- |
| 食品 | 光泉無加糖濃豆漿 |
| 品牌／情境 | 全家 FamilyMart |
| 分類 | 豆漿 |
| 份量基準 | 每份375毫升（本包裝含1份） |
| energy_kcal | 190.2 |
| protein_g | 19.1 |
| fat_g | 9.8 |
| carbohydrate_g | 7.9 |

# Review Required

升為 stable 前，真人 reviewer 必須確認產品身分、份量基準、來源版本、營養數值與重複／改版關係。

[^sheet-source-7ae5a08dc447]: 光泉無加糖濃豆漿－全家食在購安心
