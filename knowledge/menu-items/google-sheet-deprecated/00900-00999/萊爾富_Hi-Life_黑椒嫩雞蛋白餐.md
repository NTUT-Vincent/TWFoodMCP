---
type: Food Product
title: 萊爾富 Hi-Life 黑椒嫩雞蛋白餐
description: 從 TWFood 共筆試算表匯入的可追溯營養觀測 draft。
resource: https://www.instagram.com/reel/C7OePJHheWF/
tags:
  - 萊爾富 Hi-Life
  - 餐盒
  - TWFood 共筆
  - 待人工審核
generated:
  by: twfoodmcp-google-sheet-importer/1.0.0
  at: 2026-08-22T04:54:57.024Z
status: draft
stale_after: 2027-02-08
sources:
  - id: sheet-source-57afc12fbc07
    resource: https://www.instagram.com/reel/C7OePJHheWF/
    title: 萊爾富三款高蛋白健康便當
    source_class: community_submission
    retrieved_at: 2026-08-08T00:00:00+08:00
access:
  classification: public
food:
  id: food:tw:menu:google-sheet:nut-00997
  kind: menu_item
  market: TW
  brand: 萊爾富 Hi-Life
  name: 黑椒嫩雞蛋白餐
  aliases:
    - 黑椒嫩雞蛋白餐
    - 萊爾富 Hi-Life黑椒嫩雞蛋白餐
    - 萊爾富 Hi-Life 黑椒嫩雞蛋白餐
revision:
  revision_id: google-sheet-2026-08-22-nut-00997
serving:
  description: 1盒（貼文可辨識一份）
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
    values:
      energy_kcal: 456
      protein_g: 23
quality:
  data_quality: community_report
  completeness: partial
  confidence: low
  calculation_allowed: false
extraction:
  source_system: TWFood Google Sheet
  spreadsheet_id: 16f4_7C9pBo0d9XDhXyRmqIh-MRhSwnGWAT122H9mONY
  sheet_name: Nutrition Data
  record_id: NUT-00997
  evidence_grade: D
  verification_status_as_reported: historical_unverified
  value_method: transcribed_from_article
  source_type: nutrition_social_post
  source_publisher: Instagram
  source_author: null
  source_row:
    record_id: NUT-00997
    food_name: 黑椒嫩雞蛋白餐
    brand_or_context: 萊爾富 Hi-Life
    category: 餐盒
    serving_basis: 1盒（貼文可辨識一份）
    calories_kcal: 456
    protein_g: 23
    fat_g: null
    saturated_fat_g: null
    trans_fat_g: null
    carbohydrate_g: null
    sugar_g: null
    sodium_mg: null
    price_twd: 95
    value_method: transcribed_from_article
    evidence_grade: D
    verification_status: historical_unverified
    source_type: nutrition_social_post
    source_publisher: Instagram
    source_author: null
    source_title: 萊爾富三款高蛋白健康便當
    source_url: https://www.instagram.com/reel/C7OePJHheWF/
    article_updated_at: null
    accessed_at: 46242
    primary_source_url: null
    usage_note: 社群影片摘要列出商品名、熱量、蛋白質與價格；文章日期未能可靠判定、品項可能已改版或下架，列歷史待驗證。
source_observation:
  price_twd: 95
  usage_note: 社群影片摘要列出商品名、熱量、蛋白質與價格；文章日期未能可靠判定、品項可能已改版或下架，列歷史待驗證。
official_review_hint:
  status: not_compared_or_no_match
limitations:
  - 此文件保存試算表的一筆觀測，不表示品牌、政府或 TWFoodMCP 已人工確認其正確性。
  - 試算表中的 verification_status 只作為原始欄位保留，不會轉成 OKF verified 或 human-reviewed。
  - 此 draft 的 quality.calculation_allowed 固定為 false；缺少的營養欄位維持 unknown。
  - 份量描述依原始試算表保存，未據此推算重量、容量或每 100 g／ml 數值。
---

# Summary

此文件保存試算表記錄 NUT-00997 的營養觀測。[^sheet-source-57afc12fbc07] 它是未驗證 draft，不能直接用於營養計算。

# Observation

| 欄位 | 值 |
| --- | --- |
| 食品 | 黑椒嫩雞蛋白餐 |
| 品牌／情境 | 萊爾富 Hi-Life |
| 分類 | 餐盒 |
| 份量基準 | 1盒（貼文可辨識一份） |
| energy_kcal | 456 |
| protein_g | 23 |

# Review Required

升為 stable 前，真人 reviewer 必須確認產品身分、份量基準、來源版本、營養數值與重複／改版關係。

[^sheet-source-57afc12fbc07]: 萊爾富三款高蛋白健康便當
