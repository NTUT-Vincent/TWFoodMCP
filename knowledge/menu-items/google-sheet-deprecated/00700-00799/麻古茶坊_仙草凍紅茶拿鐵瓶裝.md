...
food:
  id: food:tw:menu:google-sheet:nut-00717
  kind: menu_item
  market: TW
  brand: 麻古茶坊
  name: 仙草凍紅茶拿鐵瓶裝
  aliases:
    ...
    - 麻古茶坊 仙草凍紅茶拿鐵瓶裝
revision:
  revision_id: google-sheet-2026-08-22-nut-00717
serving:
...
extraction:
  ...
  source_row:
    ...
    usage_note: 文章表格轉錄總熱量與總糖；咖啡因標示 >=201 mg。容量、甜度與冰量未明示，不宜和其他杯型直接比較。
source_observation:
  usage_note: 文章表格轉錄總熱量與總糖；咖啡因標示 >=201 mg。容量、甜度與冰量未明示，不宜和其他杯型直接比較。
...
此文件保存試算表記錄 NUT-00717 的營養觀測。[^sheet-source-466715605c89] 它是未驗證 draft，不能直接用於營養計算。
