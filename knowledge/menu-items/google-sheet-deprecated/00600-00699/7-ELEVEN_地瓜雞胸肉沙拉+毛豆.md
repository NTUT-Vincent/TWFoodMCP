---
type: Food Product
title: 7-ELEVEN 地瓜雞胸肉沙拉＋毛豆
description: 從 TWFood 共筆試算表匯入的可追溯營養觀測 draft。
resource: https://events.cofit.me/blog-711-diet-combination-30954
tags:
  - 7-ELEVEN
  - 營養師搭配組合
  - TWFood 共筆
  - 待人工審核
generated:
  by: twfoodmcp-google-sheet-importer/1.0.0
  at: 2026-08-22T04:54:57.024Z
status: draft
stale_after: 2027-02-06
sources:
  - id: sheet-source-e0c94e0d047c
    resource: https://events.cofit.me/blog-711-diet-combination-30954
    title: 節稅季如何省？只能吃超商？7-11百元超值組合，營養師教你挑！
    source_class: expert_interpretation
    retrieved_at: 2026-08-06T00:00:00+08:00
access:
  classification: public
food:
  id: food:tw:menu:google-sheet:nut-00606
  kind: menu_item
  market: TW
  brand: 7-ELEVEN
  name: 地瓜雞胸肉沙拉＋毛豆
  aliases:
    - 地瓜雞胸肉沙拉＋毛豆
    - 7-ELEVEN地瓜雞胸肉沙拉＋毛豆
    - 7-ELEVEN 地瓜雞胸肉沙拉＋毛豆
revision:
  revision_id: google-sheet-2026-08-22-nut-00606
serving:
  description: 1組（文章完整搭配）
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
    values:
      energy_kcal: 197
      protein_g: 18.2
      fat_g: 7.6
      carbohydrate_g: 14
quality:
  data_quality: third_party_database
  completeness: partial
  confidence: low
  calculation_allowed: false
extraction:
  source_system: TWFood Google Sheet
  spreadsheet_id: 16f4_7C9pBo0d9XDhXyRmqIh-MRhSwnGWAT122H9mONY
  sheet_name: Nutrition Data
  record_id: NUT-00606
  evidence_grade: C
  verification_status_as_reported: unverified
  value_method: transcribed_from_article
  source_type: dietitian_article
  source_publisher: Cofit 我的專屬營養師
  source_author: 吳悦慈營養師
  source_row:
    record_id: NUT-00606
    food_name: 地瓜雞胸肉沙拉＋毛豆
    brand_or_context: 7-ELEVEN
    category: 營養師搭配組合
    serving_basis: 1組（文章完整搭配）
    calories_kcal: 197
    protein_g: 18.2
    fat_g: 7.6
    saturated_fat_g: null
    trans_fat_g: null
    carbohydrate_g: 14
    sugar_g: null
    sodium_mg: null
    price_twd: 111
    value_method: transcribed_from_article
    evidence_grade: C
    verification_status: unverified
    source_type: dietitian_article
    source_publisher: Cofit 我的專屬營養師
    source_author: 吳悦慈營養師
    source_title: 節稅季如何省？只能吃超商？7-11百元超值組合，營養師教你挑！
    source_url: https://events.cofit.me/blog-711-diet-combination-30954
    article_updated_at: null
    accessed_at: 46240
    primary_source_url: null
    usage_note: 營養師文章計算的整組搭配值，非 7-ELEVEN 官方營養標示；品項、價格與販售狀態可能已改變，應以現行包裝核對。
source_observation:
  price_twd: 111
  usage_note: 營養師文章計算的整組搭配值，非 7-ELEVEN 官方營養標示；品項、價格與販售狀態可能已改變，應以現行包裝核對。
official_review_hint:
  status: not_compared_or_no_match
limitations:
  - 此文件保存試算表的一筆觀測，不表示品牌、政府或 TWFoodMCP 已人工確認其正確性。
  - 試算表中的 verification_status 只作為原始欄位保留，不會轉成 OKF verified 或 human-reviewed。
  - 此 draft 的 quality.calculation_allowed 固定為 false；缺少的營養欄位維持 unknown。
  - 份量描述依原始試算表保存，未據此推算重量、容量或每 100 g／ml 數值。
---

# Summary

此文件保存試算表記錄 NUT-00606 的營養觀測。[^sheet-source-e0c94e0d047c] 它是未驗證 draft，不能直接用於營養計算。

# Observation

| 欄位 | 值 |
| --- | --- |
| 食品 | 地瓜雞胸肉沙拉＋毛豆 |
| 品牌／情境 | 7-ELEVEN |
| 分類 | 營養師搭配組合 |
| 份量基準 | 1組（文章完整搭配） |
| energy_kcal | 197 |
| protein_g | 18.2 |
| fat_g | 7.6 |
| carbohydrate_g | 14 |

# Review Required

升為 stable 前，真人 reviewer 必須確認產品身分、份量基準、來源版本、營養數值與重複／改版關係。

[^sheet-source-e0c94e0d047c]: 節稅季如何省？只能吃超商？7-11百元超值組合，營養師教你挑！
