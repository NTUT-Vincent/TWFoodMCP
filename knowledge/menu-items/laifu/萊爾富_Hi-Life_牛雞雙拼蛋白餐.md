---
type: Food Product
title: 萊爾富 Hi-Life 牛雞雙拼蛋白餐
description: 從 TWFood 共筆試算表匯入的可追溯營養觀測 draft。
resource: https://www.ptt.cc/bbs/CVS/M.1740133800.A.084.html
tags:
  - 萊爾富 Hi-Life
  - 餐盒
  - TWFood 共筆
  - 待人工審核
generated:
  by: twfoodmcp-google-sheet-importer/1.0.0
  at: 2026-08-22T04:54:57.024Z
status: draft
stale_after: 2027-02-18
sources:
  - id: sheet-source-6a997ad6b58d
    resource: https://www.ptt.cc/bbs/CVS/M.1740133800.A.084.html
    title: "[商品] 萊爾富牛雞雙拼蛋白餐"
    source_class: community_submission
    last_modified: 2025-02-21T00:00:00+08:00
    retrieved_at: 2026-08-18T00:00:00+08:00
access:
  classification: public
food:
  id: food:tw:menu:laifu:hi-life_牛雞雙拼蛋白餐
  kind: menu_item
  market: TW
  brand: 萊爾富 Hi-Life
  name: 牛雞雙拼蛋白餐
  aliases:
    - 牛雞雙拼蛋白餐
    - 萊爾富 Hi-Life牛雞雙拼蛋白餐
    - 萊爾富 Hi-Life 牛雞雙拼蛋白餐
revision:
  revision_id: google-sheet-2026-08-22-nut-01165
serving:
  description: 1盒（完整商品）
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
    values:
      energy_kcal: 489
quality:
  data_quality: community_report
  completeness: minimal
  confidence: low
  calculation_allowed: false
extraction:
  source_system: TWFood Google Sheet
  spreadsheet_id: 16f4_7C9pBo0d9XDhXyRmqIh-MRhSwnGWAT122H9mONY
  sheet_name: Nutrition Data
  record_id: NUT-01165
  evidence_grade: D
  verification_status_as_reported: historical_unverified
  value_method: transcribed_from_forum_post
  source_type: forum_post
  source_publisher: PTT 批踢踢實業坊
  source_author: smile222
  source_row:
    record_id: NUT-01165
    food_name: 牛雞雙拼蛋白餐
    brand_or_context: 萊爾富 Hi-Life
    category: 餐盒
    serving_basis: 1盒（完整商品）
    calories_kcal: 489
    protein_g: null
    fat_g: null
    saturated_fat_g: null
    trans_fat_g: null
    carbohydrate_g: null
    sugar_g: null
    sodium_mg: null
    price_twd: 99
    value_method: transcribed_from_forum_post
    evidence_grade: D
    verification_status: historical_unverified
    source_type: forum_post
    source_publisher: PTT 批踢踢實業坊
    source_author: smile222
    source_title: "[商品] 萊爾富牛雞雙拼蛋白餐"
    source_url: https://www.ptt.cc/bbs/CVS/M.1740133800.A.084.html
    article_updated_at: 45709
    accessed_at: 46252
    primary_source_url: null
    usage_note: 論壇開箱明示整盒489大卡與99元；蛋白質單位誤寫為「34卡」，無法確認時不填蛋白質欄。
source_observation:
  price_twd: 99
  usage_note: 論壇開箱明示整盒489大卡與99元；蛋白質單位誤寫為「34卡」，無法確認時不填蛋白質欄。
official_review_hint:
  status: not_compared_or_no_match
limitations:
  - 此文件保存試算表的一筆觀測，不表示品牌、政府或 TWFoodMCP 已人工確認其正確性。
  - 試算表中的 verification_status 只作為原始欄位保留，不會轉成 OKF verified 或 human-reviewed。
  - 此 draft 的 quality.calculation_allowed 固定為 false；缺少的營養欄位維持 unknown。
  - 份量描述依原始試算表保存，未據此推算重量、容量或每 100 g／ml 數值。
---

# Summary

此文件保存試算表記錄 NUT-01165 的營養觀測。[^sheet-source-6a997ad6b58d] 它是未驗證 draft，不能直接用於營養計算。

# Observation

| 欄位 | 值 |
| --- | --- |
| 食品 | 牛雞雙拼蛋白餐 |
| 品牌／情境 | 萊爾富 Hi-Life |
| 分類 | 餐盒 |
| 份量基準 | 1盒（完整商品） |
| energy_kcal | 489 |

# Review Required

升為 stable 前，真人 reviewer 必須確認產品身分、份量基準、來源版本、營養數值與重複／改版關係。

[^sheet-source-6a997ad6b58d]: [商品] 萊爾富牛雞雙拼蛋白餐
