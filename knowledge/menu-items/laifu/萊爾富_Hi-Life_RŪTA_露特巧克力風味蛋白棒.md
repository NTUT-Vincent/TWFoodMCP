---
type: Food Product
title: 萊爾富 Hi-Life RŪTA 露特巧克力風味蛋白棒
description: 從 TWFood 共筆試算表匯入的可追溯營養觀測 draft。
resource: https://www.facebook.com/nuturexfit/videos/4249254935315646/
tags:
  - 萊爾富 Hi-Life
  - 蛋白棒
  - TWFood 共筆
  - 待人工審核
generated:
  by: twfoodmcp-google-sheet-importer/1.0.0
  at: 2026-08-22T04:54:57.024Z
status: draft
stale_after: 2027-02-11
sources:
  - id: sheet-source-f40328967f59
    resource: https://www.facebook.com/nuturexfit/videos/4249254935315646/
    title: 下午茶想吃甜食？RŪTA 露特巧克力風味蛋白棒
    source_class: community_submission
    retrieved_at: 2026-08-11T00:00:00+08:00
access:
  classification: public
food:
  id: food:tw:menu:laifu:hi-life_rta_露特巧克力風味蛋白棒
  kind: menu_item
  market: TW
  brand: 萊爾富 Hi-Life
  name: RŪTA 露特巧克力風味蛋白棒
  aliases:
    - RŪTA 露特巧克力風味蛋白棒
    - 萊爾富 Hi-LifeRŪTA 露特巧克力風味蛋白棒
    - 萊爾富 Hi-Life RŪTA 露特巧克力風味蛋白棒
revision:
  revision_id: google-sheet-2026-08-22-nut-01086
serving:
  description: 1條
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
    values:
      energy_kcal: 130
      protein_g: 10
quality:
  data_quality: community_report
  completeness: partial
  confidence: low
  calculation_allowed: false
extraction:
  source_system: TWFood Google Sheet
  spreadsheet_id: 16f4_7C9pBo0d9XDhXyRmqIh-MRhSwnGWAT122H9mONY
  sheet_name: Nutrition Data
  record_id: NUT-01086
  evidence_grade: D
  verification_status_as_reported: historical_unverified
  value_method: transcribed_from_article
  source_type: nutrition_social_post
  source_publisher: NutureFit 營養專科
  source_author: null
  source_row:
    record_id: NUT-01086
    food_name: RŪTA 露特巧克力風味蛋白棒
    brand_or_context: 萊爾富 Hi-Life
    category: 蛋白棒
    serving_basis: 1條
    calories_kcal: 130
    protein_g: 10
    fat_g: null
    saturated_fat_g: null
    trans_fat_g: null
    carbohydrate_g: null
    sugar_g: null
    sodium_mg: null
    price_twd: null
    value_method: transcribed_from_article
    evidence_grade: D
    verification_status: historical_unverified
    source_type: nutrition_social_post
    source_publisher: NutureFit 營養專科
    source_author: null
    source_title: 下午茶想吃甜食？RŪTA 露特巧克力風味蛋白棒
    source_url: https://www.facebook.com/nuturexfit/videos/4249254935315646/
    article_updated_at: null
    accessed_at: 46245
    primary_source_url: null
    usage_note: 營養社群影片明列每條130大卡、蛋白質「近10克」；蛋白質為約值，未提供完整包裝標示，且資料較舊，需核對現行商品。
source_observation:
  usage_note: 營養社群影片明列每條130大卡、蛋白質「近10克」；蛋白質為約值，未提供完整包裝標示，且資料較舊，需核對現行商品。
official_review_hint:
  status: not_compared_or_no_match
limitations:
  - 此文件保存試算表的一筆觀測，不表示品牌、政府或 TWFoodMCP 已人工確認其正確性。
  - 試算表中的 verification_status 只作為原始欄位保留，不會轉成 OKF verified 或 human-reviewed。
  - 此 draft 的 quality.calculation_allowed 固定為 false；缺少的營養欄位維持 unknown。
  - 份量描述依原始試算表保存，未據此推算重量、容量或每 100 g／ml 數值。
---

# Summary

此文件保存試算表記錄 NUT-01086 的營養觀測。[^sheet-source-f40328967f59] 它是未驗證 draft，不能直接用於營養計算。

# Observation

| 欄位 | 值 |
| --- | --- |
| 食品 | RŪTA 露特巧克力風味蛋白棒 |
| 品牌／情境 | 萊爾富 Hi-Life |
| 分類 | 蛋白棒 |
| 份量基準 | 1條 |
| energy_kcal | 130 |
| protein_g | 10 |

# Review Required

升為 stable 前，真人 reviewer 必須確認產品身分、份量基準、來源版本、營養數值與重複／改版關係。

[^sheet-source-f40328967f59]: 下午茶想吃甜食？RŪTA 露特巧克力風味蛋白棒
