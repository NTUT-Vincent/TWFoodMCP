---
type: Food Product
title: 萊爾富 Hi-Life 紐奧良烤雞三明治
description: 從 TWFood 共筆試算表匯入的可追溯營養觀測 draft。
resource: https://www.threads.com/@raymol308/post/DQq-9HSiZdq/
tags:
  - 萊爾富 Hi-Life
  - 三明治
  - TWFood 共筆
  - 待人工審核
generated:
  by: twfoodmcp-google-sheet-importer/1.0.0
  at: 2026-08-22T04:54:57.024Z
status: draft
stale_after: 2027-02-12
sources:
  - id: sheet-source-1868712e0073
    resource: https://www.threads.com/@raymol308/post/DQq-9HSiZdq/
    title: 紐奧良烤雞三明治熱量標示疑義與品牌回覆
    source_class: community_submission
    last_modified: 2025-11-05T00:00:00+08:00
    retrieved_at: 2026-08-12T00:00:00+08:00
access:
  classification: public
food:
  id: food:tw:menu:laifu:hi-life_紐奧良烤雞三明治
  kind: menu_item
  market: TW
  brand: 萊爾富 Hi-Life
  name: 紐奧良烤雞三明治
  aliases:
    - 紐奧良烤雞三明治
    - 萊爾富 Hi-Life紐奧良烤雞三明治
    - 萊爾富 Hi-Life 紐奧良烤雞三明治
revision:
  revision_id: google-sheet-2026-08-22-nut-01099
serving:
  description: 1份商品
  amount: 1
  unit: serving
nutrition:
  - basis: per_serving
    values:
      energy_kcal: 196
quality:
  data_quality: community_report
  completeness: minimal
  confidence: low
  calculation_allowed: false
extraction:
  source_system: TWFood Google Sheet
  spreadsheet_id: 16f4_7C9pBo0d9XDhXyRmqIh-MRhSwnGWAT122H9mONY
  sheet_name: Nutrition Data
  record_id: NUT-01099
  evidence_grade: B
  verification_status_as_reported: needs_verification
  value_method: transcribed_from_article
  source_type: forum_social_post
  source_publisher: Threads
  source_author: "@raymol308"
  source_row:
    record_id: NUT-01099
    food_name: 紐奧良烤雞三明治
    brand_or_context: 萊爾富 Hi-Life
    category: 三明治
    serving_basis: 1份商品
    calories_kcal: 196
    protein_g: null
    fat_g: null
    saturated_fat_g: null
    trans_fat_g: null
    carbohydrate_g: null
    sugar_g: null
    sodium_mg: null
    price_twd: null
    value_method: transcribed_from_article
    evidence_grade: B
    verification_status: needs_verification
    source_type: forum_social_post
    source_publisher: Threads
    source_author: "@raymol308"
    source_title: 紐奧良烤雞三明治熱量標示疑義與品牌回覆
    source_url: https://www.threads.com/@raymol308/post/DQq-9HSiZdq/
    article_updated_at: 45966
    accessed_at: 46246
    primary_source_url: null
    usage_note: 貼文指出包裝／宣傳曾出現304與196大卡衝突；萊爾富官方帳號於回覆確認正確熱量為196大卡。未取得更正後完整包裝標示，故仍需人工核對。
source_observation:
  usage_note: 貼文指出包裝／宣傳曾出現304與196大卡衝突；萊爾富官方帳號於回覆確認正確熱量為196大卡。未取得更正後完整包裝標示，故仍需人工核對。
official_review_hint:
  status: not_compared_or_no_match
limitations:
  - 此文件保存試算表的一筆觀測，不表示品牌、政府或 TWFoodMCP 已人工確認其正確性。
  - 試算表中的 verification_status 只作為原始欄位保留，不會轉成 OKF verified 或 human-reviewed。
  - 此 draft 的 quality.calculation_allowed 固定為 false；缺少的營養欄位維持 unknown。
  - 份量描述依原始試算表保存，未據此推算重量、容量或每 100 g／ml 數值。
---

# Summary

此文件保存試算表記錄 NUT-01099 的營養觀測。[^sheet-source-1868712e0073] 它是未驗證 draft，不能直接用於營養計算。

# Observation

| 欄位 | 值 |
| --- | --- |
| 食品 | 紐奧良烤雞三明治 |
| 品牌／情境 | 萊爾富 Hi-Life |
| 分類 | 三明治 |
| 份量基準 | 1份商品 |
| energy_kcal | 196 |

# Review Required

升為 stable 前，真人 reviewer 必須確認產品身分、份量基準、來源版本、營養數值與重複／改版關係。

[^sheet-source-1868712e0073]: 紐奧良烤雞三明治熱量標示疑義與品牌回覆
